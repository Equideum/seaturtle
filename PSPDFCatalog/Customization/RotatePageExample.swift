...
class RotatePageExample: PSCExample {
    ...
    override func invoke(with delegate: PSCExampleRunnerDelegate) -> UIViewController {
        ...
        let controller = RotatePagePDFViewController(document: document, configuration: configuration)
        return controller
    }

    // MARK: Controller

    class RotatePagePDFViewController: PSPDFViewController {
        ...
        @objc func rotatePage() {
            ...
            PSPDFKit.sharedInstance.cache.invalidateImage(from: document, pageIndex: currentPageIndex)

            // Reloading the data is required because the document is currently displayed in a `PSPDFViewController`.
            reloadData()
        }
    }
}
