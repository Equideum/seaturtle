//
//  PageViewControllerExample.swift
//  PSPDFCatalog
//
...
class PageViewControllerExample: PSCExample {
    ...
    override func invoke(with delegate: PSCExampleRunnerDelegate) -> UIViewController {
        ...
        let files: [PSCAssetName] = [.quickStart, .about, .web, .caseStudyBox, .JKHF, .annualReport]
        let documents = files.map { asset in
            return PSPDFDocument(url: samplesURL.appendingPathComponent(asset.rawValue))
        }
        ...
        let configuration = PSPDFConfiguration { builder in
            ...
            builder.userInterfaceViewMode = .automaticNoFirstLastPage
        ...
        // allocating many PSPDFViewController instances is memory intensive
        ...
        PSPDFUserInterfaceView.appearance(whenContainedInInstancesOf: ([PageViewController.self])).pageLabelInsets = UIEdgeInsets(top: 0, left: 0, bottom: 40, right: 0)

        return pageController
    }

    func updateTitle() {
        // Title is set a bit later in PSPDFViewController initialization, so we use it from the
        // document directly. Aternatively, title could be KVO ovserved.
        guard let pdfController = controller[pageIndex] as? PSPDFViewController else { return }
