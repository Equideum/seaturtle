...
- (void)commonInitWithPDFController:(nullable PSPDFViewController *)pdfController {
    ...
    self.documentPickerController = [[PSPDFDocumentPickerController alloc] initWithDirectory:@"/Bundle/Samples" includeSubdirectories:YES library:PSPDFKit.sharedInstance.library];

    _clearTabsButtonItem = [[UIBarButtonItem alloc] initWithImage:[PSPDFKit imageNamed:@"trash"] style:UIBarButtonItemStylePlain target:self action:@selector(clearTabsButtonPressed:)];
...
- (void)clearTabsButtonPressed:(id)sender {
    ...
    [sheetController addAction:[UIAlertAction actionWithTitle:NSLocalizedString(@"Cancel", @"") style:UIAlertActionStyleCancel handler:NULL]];
